...
router.post('/', async (req, res) => {
  try {
    const { type, location, address, vehicleDetails, description, contactName, contactPhone } = req.body;
    ...
    } catch (_) { /* ignore geo query errors when location missing */ }
    ...
    const withTech = await Service.findById(service._id)
...
router.get('/assigned/me', authTechnician, async (req, res) => {
  try {
    const services = await Service.find({ assignedTechnician: req.technician._id })
      .sort({ createdAt: -1 })
      .populate('assignedTechnician', 'name phone rating');
    res.json(services);
  } catch (error) {
...
router.put('/:id/rate', async (req, res) => {
  try {
    ...
    if (service.status !== 'completed') {
      return res.status(400).json({ error: 'Cannot rate a service that is not completed' });
    }
    ...
    if (service.assignedTechnician) {
      ...
      if (technician) {
        ...
        technician.rating = ((currentRating * (totalServices - 1)) + rating) / totalServices;
        await technician.save();
      }
    }
...
router.put('/:id/assign/self', authTechnician, async (req, res) => {
  try {
    ...
    res.json(await Service.findById(service._id).populate('assignedTechnician','name phone rating'));
  } catch (error) {
